...
static void handle_input(void) {
    ...
    if (input_pressed(&g_input, BTN_START)) {
        g_settings.theme        = theme_get();
        g_settings.default_mode = g_dash_mode;
    ...
    while (g_running) {
        ...
        switch (g_state) {
            case APP_STATE_WIFI_CONNECTING:
                render_connecting("Connecting to WiFi...");
                if (wifi_init() == 0 && wifi_connect(g_settings.ap_config_idx) == 0)
                    g_state = APP_STATE_OBD_CONNECTING;
                else
                    render_connecting("WiFi failed. Retrying...");
                break;

            case APP_STATE_OBD_CONNECTING:
                render_connecting("Connecting to OBD adapter...");
            ...
            case APP_STATE_RUNNING:
                ...
                /* Read DTCs once on first connection */
            ...
            case APP_STATE_ERROR:
                render_connecting("Fatal error. Press START to exit.");
                if (input_pressed(&g_input, BTN_START))
                    g_running = 0;
                break;
